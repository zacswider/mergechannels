...
def test_bench_serial_u8(benchmark, images_512x512_u8) -> None:
    """Benchmark serial colormap application on 100 512x512 uint8 images"""
    results = benchmark(apply_colormap_serial, images_512x512_u8)
    assert len(results) == 100
    assert all(r.shape[:2] == (512, 512) for r in results)


@pytest.mark.benchmark(group='threading u8 serial vs parallel')
def test_bench_threadpool_2workers_u8(benchmark, images_512x512_u8) -> None:
    """Benchmark ThreadPoolExecutor (2 workers) colormap application on 100 512x512 uint8 images"""
    results = benchmark(apply_colormap_threadpool, images_512x512_u8, max_workers=2)
    assert len(results) == 100
    assert all(r.shape[:2] == (512, 512) for r in results)


@pytest.mark.benchmark(group='threading u8 serial vs parallel')
def test_bench_threadpool_4workers_u8(benchmark, images_512x512_u8) -> None:
    """Benchmark ThreadPoolExecutor (4 workers) colormap application on 100 512x512 uint8 images"""
    results = benchmark(apply_colormap_threadpool, images_512x512_u8, max_workers=4)
    assert len(results) == 100
    assert all(r.shape[:2] == (512, 512) for r in results)


@pytest.mark.benchmark(group='threading u8 serial vs parallel')
def test_bench_threadpool_8workers_u8(benchmark, images_512x512_u8) -> None:
    """Benchmark ThreadPoolExecutor (8 workers) colormap application on 100 512x512 uint8 images"""
    results = benchmark(apply_colormap_threadpool, images_512x512_u8, max_workers=8)
    assert len(results) == 100
    assert all(r.shape[:2] == (512, 512) for r in results)


@pytest.mark.benchmark(group='threading u16 serial vs parallel')
def test_bench_serial_u16(benchmark, images_512x512_u16) -> None:
    """Benchmark serial colormap application on 100 512x512 uint16 images"""
    results = benchmark(apply_colormap_serial, images_512x512_u16)
    assert len(results) == 100
    assert all(r.shape[:2] == (512, 512) for r in results)


@pytest.mark.benchmark(group='threading u16 serial vs parallel')
def test_bench_threadpool_2workers_u16(benchmark, images_512x512_u16) -> None:
    """Benchmark ThreadPoolExecutor (2 workers) colormap application on 100 512x512 uint16 images"""
    results = benchmark(apply_colormap_threadpool, images_512x512_u16, max_workers=2)
    assert len(results) == 100
    assert all(r.shape[:2] == (512, 512) for r in results)


@pytest.mark.benchmark(group='threading u16 serial vs parallel')
def test_bench_threadpool_4workers_u16(benchmark, images_512x512_u16) -> None:
    """Benchmark ThreadPoolExecutor (4 workers) colormap application on 100 512x512 uint16 images"""
    results = benchmark(apply_colormap_threadpool, images_512x512_u16, max_workers=4)
    assert len(results) == 100
    assert all(r.shape[:2] == (512, 512) for r in results)


@pytest.mark.benchmark(group='threading u16 serial vs parallel')
def test_bench_threadpool_8workers_u16(benchmark, images_512x512_u16) -> None:
    """Benchmark ThreadPoolExecutor (8 workers) colormap application on 100 512x512 uint16 images"""
    results = benchmark(apply_colormap_threadpool, images_512x512_u16, max_workers=8)
    assert len(results) == 100
    assert all(r.shape[:2] == (512, 512) for r in results)


# Non-benchmark timing tests for manual inspection
def test_timing_comparison_u8(images_512x512_u8) -> None:
    """Manual timing comparison for serial vs threadpool execution (uint8)"""
    # Serial
    start = time.perf_counter()
    serial_results = apply_colormap_serial(images_512x512_u8)
    serial_time = time.perf_counter() - start

    # ThreadPoolExecutor with 4 workers
    start = time.perf_counter()
    threadpool_results = apply_colormap_threadpool(images_512x512_u8, max_workers=4)
...
def test_timing_comparison_u16(images_512x512_u16) -> None:
    """Manual timing comparison for serial vs threadpool execution (uint16)"""
    # Serial
    start = time.perf_counter()
    serial_results = apply_colormap_serial(images_512x512_u16)
    serial_time = time.perf_counter() - start

    # ThreadPoolExecutor with 4 workers
    start = time.perf_counter()
    threadpool_results = apply_colormap_threadpool(images_512x512_u16, max_workers=4)
    threadpool_time = time.perf_counter() - start
