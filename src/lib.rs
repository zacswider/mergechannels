...
use interface::{
    dispatch_multi_channel_py, dispatch_multi_channel_v2_py, dispatch_single_channel_py,
    dispatch_single_channel_v2_py, get_cmap_array_py,
};
use pyo3::prelude::*;
...
fn mergechannels(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    // V2 functions for benchmarking refactored implementation
    m.add_function(wrap_pyfunction!(dispatch_single_channel_v2_py, m)?)?;
    m.add_function(wrap_pyfunction!(dispatch_multi_channel_v2_py, m)?)?;
    Ok(())
}
